...
//      	- mount : 완전 초기상태 (막 들어온 상태, F5로 새로고침을 한 상태)
...
//		- useCallback : 메모리 내에서 함수를 재사용할 수 있도록 하는 함수, 랜더링 없이 특정 함수를 다시 호출 할 수 있도록 해줌
//		- useMemo : 메모리에 계산된 값(함수의 결과)를 저장하여 함수 재실행 없이 사용할 수 있도록 하는 함수

//	- hook 함수는 반드시 컴포넌트의 코드 블럭에 존재해야함
//	- 만약 컴포넌트의 코트 블럭 외에 존재하고 싶다면 그 코드 블럭은 반드시 훅 함수이어야 함
...
// 스코프 할 상태 배열을 지정하지 않으면 모든 상태가 변경될 때마다 실행됨
	useEffect(() => {
		// 스코프 할 상태 배열을 지정하지 않은 effect에서 상태를 변경하면 무한 실행됨
		// setFlag(!flag);
