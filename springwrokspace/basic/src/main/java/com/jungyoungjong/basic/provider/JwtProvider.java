package com.jungyoungjong.basic.provider;
...
// JWT :
// - Json Web Token, RFC 7519 표준에 정의된 json형식의 문자열을 포함하고 있는 토큰
// - 인증 및 인가에 사용
// - 암호화가 되어 있어 클라이언트와 서버 간의 안전한 데이터 교환을 할 수 있음
// - 헤더, 페이로드, 서명으로 구성됨
//      - 헤더 : 토큰의 유형(일반적으로 jwt)과 암호화 알고리즘이 포함되어있음
//      - 페이로드 : 클라이언트 혹은 서버가 상대방에게 전잘할 데이터가 포함되어있음(작성자, 접근주체의 정보, 작성시간, 만료시간)
...
public class JwtProvider {
    ...
    public String create(String principle) {
        ...
        // 비밀키 생성
        Key key = Keys.hmacShaKeyFor("qweasdzxcvqweasdzxcvqweasdzxcvqweasdzxcv".getBytes(StandardCharsets.UTF_8));
