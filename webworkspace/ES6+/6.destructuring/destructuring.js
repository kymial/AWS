/*
    디스트럭처링 :
    - 구조화된 배열 또는 객체를 비구조화, 구조화 파괴하여 개볇젼수로 활당하는 것
    -배열 또는 객체에서 필요한 데이터만 추출해서 변수에 활당하여 사용할 수 있도록 함
*/
let king = {
    name : '이성계',
    tomName : '태조',
...
/*
    spread 연산자를 활용
    - 배열 디스스트럭처링 마지막에 스프레드 연산자를 사용하면 나머지 모든 요소로 새로운 배열을 생성
*/
...
// 객체 디스트럭처링에서는 변수의 순서는 상관 없지만, 이름은 반드시 객체의 키의 이름과 같아야함
const {name, tomName} = king;
console.log(name);
console.log(tomName);
