...
//      - Redux, MobX 와 같은 타 상태관리 라이브러리들에 비해 번들의 크기가 작음    - 빌드할 때 패키징 속도 향상    / 빌드 후 파일의 크기가 작아짐
...
//  - create 함수의 매개변수로 set 인자를 받는 콜백 함수로 전달해야 함
//  - 매개변수로 전달한 콜백 함수는 store 객체를 반환해야 함
//  - store 객체는 상태, store 객체를 변경하는 함수가 포함됨

//  - typescript에서 zustand의 create 함수를 사용할때는 create 함수의 제너릭으로 상태의 타입을 지정해야 함
interface Store {
    zNormal: number;
...
export default function ZustandLibrary() {
    ...
    <div>
        ...
        <Link to= 'http://localhost:3000/router' >홈으로</Link>
        <div>
...
function SubComponent2 () {
    ...
    const { zNormal, decreaseZNormal, increaseZNormal } = useStore();
