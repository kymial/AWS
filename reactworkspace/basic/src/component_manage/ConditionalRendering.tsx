...
function AndComponent ( { number } : { number : number }) {
    // && 연산자를 이용한 조건부 렌더링
    // - JSX 내부에서 상황(조건)에 따라 다른 렌더링 여부을 결정 하고 싶을 때 사용

    // ex) 만약 number가 양수면 '양수'를 랜더링
    // if (number > 0 ) return '양수';
    // 양수가 아니면 '양수가 아님'을 랜더링
...
function ThereTermComponent ( { number } : { number : number }) {
...
export default function ConditionalRendering() {
    return (
        <>
            ...
            <ThereTermComponent number = {1} />
            <ThereTermComponent number = {0} />
            <ThereTermComponent number = {-1} />
        </>
    )
}
