...
클라이언트로부터 Request Header의 Authorization 필드로 Bearer 토큰을 포함하고 제목, 내용을 입력받고 성공처리를 합니다.
...
클라이언트로부터 Request Header의 Authorization 필드로 Bearer 토큰을 포함하여 요청을 보내면 작성일 기준 내림차순으로 게시물 리스트를 반환합니다. 만약 불러오기에 실패하면 실패처리를 합니다.
...
| writerDatetime | String | 작성일</br> (yy.mm.dd 형태) | O |
| viewCount | int | 조회수 | O |
