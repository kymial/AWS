...
export default function StateComponent() {
  ...
  // let favorites: string[] = [];
    ...
    const onCountAddHandler = () => {
        ...
        // 상태변수는 반드시 상태변경함수로 변경해야 리젠더링됨
        // count++;
        
        // 상태변경 함수를 통해서 함수를 변경한다고 바로 적용되지 않음
        // 리젠더링된 후 상태변경 함수가 적용됨
        // setCount(count + 1);
        // setCount(count + 1);
        // setCount(count + 1);
    ...
    const onAddListHandler = () => {
        // 상태가 배열 혹은 객체이면 각각에 대한 요소를 추가, 변경이 일어나도 상태가 변경된 것으로 인식하지 않음 (실제 주소 변경 X)
        // 상태가 변경된 것으로 인식하게 하고 싶으면 새로운 배열 혹은 객체를 생성하여 상태를 변경해야 한다. (일반적으로 복사해서 변경)
