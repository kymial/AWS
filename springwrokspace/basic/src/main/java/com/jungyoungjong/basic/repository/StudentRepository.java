package com.jungyoungjong.basic.repository;
...
// - Service가 비즈니스 로직 수행 중 데이터베이스 작업이 필요할 때 Repository 를 거쳐서 데이터베이스 작업을 수행
...
// - 기본 CRUD, 정렬 기능에 제공하고 있음
// - JPA 기반 Repository를 생성 할때 반드시 상속해야함
// - 매개타입 T : 해당 Repository가 어떤 Entity의 Repository인지 나타내는 매개타입
// - 매개타입 ID : 해당 Repository에서 사용하는 Entity의 Primary key 데이터 타입을 저장하는 매개타입
extends JpaRepository<StudentEntity, Integer> {
    ...
    // @query :
    // - 쿼리 메서드 생성 방식으로는 실제 SQL을 작성하는 데 한계가 있따.
    // - 쿼리 메서드는 복잡한 쿼리, 조인, 서브쿼리, 그룹화를 사용할 수 없음
    // - 직접 SQL문으로 쿼리를 생성하는 하는 어노테이션
