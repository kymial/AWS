...
// 규칙
// 1. 상위 클래스에서 선언된 메서드와 메서드 이름, 매개벼수의 조합이 같아야함
// 2. 상위 클래스에서 선언된 메서드가 final로 선언되면 재정의가 불가능
// 3. 상위 클래스에서 선언된 메서드 보다 더 좁은 범위의 접근제어를 할 수 없음
...
class SuperClass1{
	...
	void InstanceMethod() {
		System.out.println("조상 클래스의 메소드 입니다.");
	}
}
...
class SubClass1 extends SuperClass1{
	...
	// SuperClass1을 상속한 상태에서 아래 메서드를 가지고 있음
	// void InstanceMethod() {...}

	// 'void InstanceMethod()' 메서드를 이미 자고 있기 때문에
	// 아래 메서드는 void InstanceMethod()' 메서드를 오버로드 한 것
	
	void InstanceMethod() {
		System.out.println("SubClass1의 인스턴스입니다.");
	}
...
class SubClass2 extends SuperClass1{
	...
	// @Override : 컴파일러에게 해당 아래에 오는 메서드가 오버라이드 메서드라는 것을 알려주는것
	// 기독성 향상과 개발자의 실수를 방지할 수 있음
	...
	void InstanceMethod() {
		...
		System.out.println("SubClass2의 인스턴스입니다.");
	}
