...
import com.jungyoungjong.basic.service.implement.BasicServiceImplement;
...
public class BasicController {
    ...
    // - 생성자를 통한 의존성 주입 spring에서 권장 (의존성이 주입되지 않은 경우가 존재할 수 없기 때문)
    // - spring framework에서 의존성 주입은 제어의 역전 (IoC)를 통해서 spring framework가 진행(@Component로 사용해서 Spring Bean으로 등록된 Class에 한해서)
    ...
    //@ PutMapping() : RequestMapping 기능을 Put Method에 한정시킨 컷 (가독성 + 안전성)
    ...
    // @PatchMapping() : RequestMapping 기능을 Patch Method에 한정시킨 컷 (가독성 + 안전성)
    ...
    // HTTP DELETE Method : 클라이언트가 서버에 있는 리소스를 삭제 하고 싶을 때 사용하는 메서드
    // @DeleteMapping() : RequestMapping 기능을 Patch Method에 한정시킨 컷 (가독성 + 안전성)
    @DeleteMapping("/apple")
    public String deleteApple(){
        return "Delete Mapping으로 만든 메서드";
    }

//  Method + URL Pattern이 중복되면 런타임 오류가 발생한다.
//  @DeleteMapping("/apple")
//  public String deleteApple(){
//      return "Delete Mapping으로 만든 메서드";
//  }
