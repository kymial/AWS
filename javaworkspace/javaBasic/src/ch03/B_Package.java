...
public class B_Package {
	...
	public static void main(String[] args) {
		...
		Integer warpperInt = 10;
		Double warpperDoulbe =10.0;
		...
		// toString 메서드 : 특정 타입 값을 문자열로 변환
		String string = Integer.toString(123);
		
		
		// Collection Framework : jave에서 데이터구조에서 대한 표준화된 기법을 제공
		
		// List : 데이터를 저장할 때 중복을 허용하고 순서가 존재함
		List<Integer> arrayList; // int[] arrayList; 비슷함
		...
		// add() : List에 요소를 추가
		// 추가할 요소에 데이터 타입은 반드시 제너리으로 지정한 타입
		arrayList.add(10);
//		arrayList.add("aaaaa");
		System.out.println(arrayList);
		...
		// List 인덱스 범위를 초과하는 인덱스를 지정할 수 없다.
		...
		// List 인덱스 범위를 초과하는 인덱르르 지정할 수 없음
		arrayList.set(0, 50);
		arrayList.set(1, -99);
		System.out.println(arrayList);
		...
		// 제거하고자 하는 요소가 없더라도 예외는 발생하지 않음
		set.remove("Apple1");
		System.out.println(set);
		
		// contains() : set에서 특정 요소가 존재하는 확인
		System.out.println(set.contains("Banana"));
		
		// Map : 키와 값이 한쌍으로 이루어지 요소들의 집합, 키는 중복 불가능, 값은 중복 가능
		Map<String, String> map;
		
		// HashMap : 해싱 기법으로 키의 검색속도가 빠름
		//map = new HashMap<>();
		...
		// get() : Map에서 특정 키를 가지고 요소를 반환
		// 존재하지 않은 키로 검색할 경우 null로 반환
		System.out.println(map.get("apple"));
		
		// remove() : Map에서 특정 키를 가지고 요소를 제거
