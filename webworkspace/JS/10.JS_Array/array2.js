...
/*
    map(콜백 함수) : 
    - 콜백 함수의 반환값들로 이루어진 새로운 배열 생성해서 반환
    - 원본 배열 변경 X
    - forEach 단순히 요소를 가지고 작업만 진행 (반환값이 없다), map 요소를 가지고 작업한 결과로 배열을 새로 만듬 (반환값이 존재한다.)
*/
...
/*
    filter(콜백 함수) : 
    - 콜백함수의 반환 값이 true인 배열의 요소의 값만 추출한 새로운 배열을 생성
    - 배열에서 특정케이스만 필터링 조건으로 추출하여 새로운 배열을 만들고 싶을 때 사용
    - 원본 배열 변경 불가능
*/
...
/*
    findIndex(콜백 함수) : 
    - 콜백 함수 결과가 true인 첫번째 요소의 인덱스를 반환
*/
