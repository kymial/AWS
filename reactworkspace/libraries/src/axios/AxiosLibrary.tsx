...
export default function AxiosLibrary() {
    ...
    axios
            .get(GET_URL)
            // 각각의 http method 함수 결과를 then으로 받을 땐 response객체를 매개변수로 받는 골백함수를 전달
            ...
            .catch(error => {
                // 해당 request에 대해서 request가 실패하거나 status code가 400, 500번대이면 catch로 AxiosError 객체로 받아옴
            ...
            // request body 매개변수
            //  - post, patch, put 함수는 두번재 매개변수로 request body 데이터를 전달함
            const PATCH_URL = 'http://localhost:4100/api/v1/user/nickname' ;
            axios.patch(PATCH_URL, { email: 'email@email.com', nickname: '하우카우' });
